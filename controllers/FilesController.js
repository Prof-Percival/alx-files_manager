...
export default class FilesController {
  ...
  static async postUpload(req, res) {
    ...
    if (!fileType || !Object.values(VALID_FILE_TYPES).includes(fileType)) {
      res.status(400).json({ error: 'Missing type' });
      return;
    }
    if (!req.body.data && fileType !== VALID_FILE_TYPES.folder) {
    ...
    if (fileType !== VALID_FILE_TYPES.folder) {
    ...
    if (fileType === VALID_FILE_TYPES.image) {
  ...
  static async getShow(req, res) {
    ...
    res.status(200).json({
      ...
      parentId: fileObject.parentId === ROOT_FOLDER_ID.toString()
        ? 0
        : fileObject.parentId.toString(),
    });
  }
  ...
  static async getFile(req, res) {
    ...
    const { id } = req.params;
    const fileSize = req.query.size || null;
    ...
    if (!fileObject || (!fileObject.isPublic && (fileObject.userId.toString() !== userId))) {
      res.status(404).json({ error: 'Not found' });
      return;
    }
    if (fileObject.type === VALID_FILE_TYPES.folder) {
      res.status(400).json({ error: 'A folder doesn\'t have content' });
      return;
    }
    let filePath = fileObject.localPath;
    if (fileSize) {
      filePath = `${fileObject.localPath}_${fileSize}`;
    }
    ...
    res.setHeader('Content-Type', contentType(fileObject.name) || 'text/plain; charset=utf-8');
